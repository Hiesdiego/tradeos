...
export async function getReputationSummary(userId: string) {
  ...
  const buyerEvents = events.filter((event) => event.trade.buyer_id === userId);
  const merchantEvents = events.filter(
    (event) => event.trade.supplier_id === userId
  );

  const scoreFromEvents = (roleEvents: typeof events): string => {
    if (roleEvents.length === 0) return "0.0";
    const score = roleEvents.reduce(
      (acc, event) => acc + Number(event.score_delta),
      5
    );
  ...
  return {
    ...
    buyer_reputation: {
      score: scoreFromEvents(buyerEvents),
      max: 5,
      trades: buyerEvents.length,
    },
    merchant_reputation: {
      score: scoreFromEvents(merchantEvents),
      max: 5,
      trades: merchantEvents.length,
    },
    events,
  };
}
