...
async function notifyTradeSubscribers(
  tradeId: string,
  message: string,
  excludeChatIds: Array<string | null | undefined> = []
): Promise<void> {
  const excluded = new Set(
    excludeChatIds.filter((v): v is string => typeof v === "string" && v.length > 0)
  );

  const subs = await prisma.telegramTradeSubscription.findMany({
    where: { trade_id: tradeId },
    select: { chat_id: true },
  });

  const uniqueChatIds = [...new Set(subs.map((s) => s.chat_id))].filter(
    (chatId) => !excluded.has(chatId)
  );

  await Promise.all(uniqueChatIds.map((chatId) => notify(chatId, message)));
}
...
export async function notifyMilestoneReleased(
  ...
  await Promise.all([
    ...
    notifyTradeSubscribers(trade.id, buyerMsg, [buyerChatId, supplierChatId]),
  ]);
}
...
export async function notifyDisputeRaised(
  ...
  await Promise.all([
    ...
    notifyTradeSubscribers(trade.id, partyMsg, [buyerChatId, supplierChatId]),
  ]);
}
...
export async function notifyDisputeResolved(
  ...
  await Promise.all([
    ...
    notifyTradeSubscribers(trade.id, msg, [buyerChatId, supplierChatId]),
  ]);
}
...
export async function notifyNewMessage(
  ...
  await notifyLinkedUser(recipientUserId, recipientChatId, message);
  await notifyTradeSubscribers(trade.id, message, [recipientChatId]);
}
