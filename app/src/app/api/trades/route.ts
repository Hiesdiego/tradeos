...
export const GET = withAuth(async (req: AuthedRequest) => {
  const activeTradeIds = await prisma.trade.findMany({
    where: {
      OR: [{ buyer_id: req.user.id }, { supplier_id: req.user.id }],
      status: { in: ["funded", "in_progress", "milestone_1_released", "milestone_2_released"] },
    },
    select: { id: true },
    take: 50,
  });
  await Promise.all(
    activeTradeIds.map(({ id }) =>
      autoEscalateProofTimeoutForTrade(id).catch(() => 0)
    )
  );
...
export const POST = withAuth(async (req: AuthedRequest) => {
  try {
    ...
    const buyerTermsAccepted = body.buyer_terms_accepted === true;
    if (!buyerTermsAccepted) {
      return NextResponse.json(
        { error: "You must accept Tradeos order terms before creating a trade" },
        { status: 400 }
      );
    }
  ...
  // Validate milestone percentages sum to 100
    const corridorGuidance = buildCorridorGuidance({
      corridor,
      commodityType: goods_category,
    });
    ...
    const milestonesToCreate =
      ...
      corridorGuidance.milestone_templates.map((m) => ({
    ...
    for (let i = 0; i < 5; i += 1) {
      try {
        ...
        return NextResponse.json(
          {
            ...trade,
            corridor_intelligence: corridorGuidance,
