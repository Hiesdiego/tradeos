...
type Dispute = {
  ...
  raiser: DisputeUser;
  panel_size?: number;
  assignments?: { arbiter_profile_id: string; arbiter?: { user_id: string } }[];
};
...
function DisputeCard({
  ...
  const isAssigned = Boolean(
    dispute.assignments?.some((a) => a.arbiter?.user_id === currentUserId)
  );
  const hasCapacity =
    typeof dispute.panel_size === "number"
      ? (dispute.assignments?.length ?? 0) < dispute.panel_size
      : false;
  const claimable = dispute.status === "open" && !isAssigned && hasCapacity;
    ...
    >
      ...
      {!isResolved && (
        <div className="border-t border-border pt-4">
          {claimable && (
            <Button
              onClick={() => onClaim(dispute.id)}
              disabled={claiming}
              className="mb-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold"
            >
          ...
          {expanded && isAssigned && (
            <div className="mt-4 space-y-4">
              ...
              <div className="space-y-2">
                <Label>Arbiter notes (required)</Label>
                <Textarea
              ...
              <Button
                ...
                disabled={resolving || !resolution || !notes.trim()}
...
export default function AdminPage() {
  ...
  const { handleResolveDispute } = useEscrow();
  ...
  const isArbiter = !accessDenied;

  const fetchDisputes = useCallback(async () => {
    try {
      const token = await getAccessToken();
      if (!token) return;
      const res = await fetch("/api/arbitration/disputes", {
  ...
  async function handleResolve(
    ...
    try {
      ...
      if (!target) return;
      const onchainTx = await handleResolveDispute(target.trade as unknown as Trade, supplierBps);
      const token = await getAccessToken();
  ...
  async function handleClaim(disputeId: string) {
    setClaiming(true);
    try {
      const token = await getAccessToken();
      const res = await fetch(`/api/disputes/${disputeId}/claim`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-xs">
          This area is only accessible to configured Tradeos admin wallets.
          Register an arbiter profile to access assigned disputes.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-2xl space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Review assigned disputes, vote, and finalize panel resolutions
        </p>
      </div>
