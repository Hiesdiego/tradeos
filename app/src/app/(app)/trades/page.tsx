...
import { useTradeActions, useTrades } from "@/hooks/useTrade";
...
function TradeCard({
  ...
  const canDelete =
    ...
    currentTimestamp - new Date(trade.created_at).getTime() >= 24 * 60 * 60 * 1000;

  return (
    <div className="trade-card group flex flex-col gap-4">
      <Link href={`/trades/${trade.id}`} className="contents">
      ...
      {canDelete && (
        <div className="pt-2 border-t border-[hsl(var(--gold)/0.2)]">
          <Button
            size="sm"
            variant="outline"
            disabled={deleting}
            onClick={() => onDelete(trade.id)}
            className="text-xs border-red-400/40 text-red-300 hover:bg-red-500/10"
          >
            {deleting ? "Deleting..." : "Delete Stale Order"}
          </Button>
        </div>
      )}
    </div>
...
export default function TradesPage() {
  ...
  const { deleteTrade, loading: actionLoading } = useTradeActions();
