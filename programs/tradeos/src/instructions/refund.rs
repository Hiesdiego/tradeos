...
pub fn handler(ctx: Context<RefundEscrow>, expected_nonce: u64) -> Result<()> {
    // ✅ Capture AccountInfo BEFORE taking the mutable borrow
    let escrow_account_info = ctx.accounts.escrow.to_account_info();

    let escrow = &mut ctx.accounts.escrow;
    require!(expected_nonce == escrow.refund_nonce, TradeOSError::InvalidNonce);
