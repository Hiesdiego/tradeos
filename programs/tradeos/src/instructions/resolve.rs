...
pub fn handler(ctx: Context<ResolveDispute>, release_to_supplier_bps: u16) -> Result<()> {
    ...
    let fee_amount = (escrow_balance as u128)
        .checked_mul(escrow.arbitration_fee_bps as u128)
        .ok_or(TradeOSError::ArithmeticOverflow)?
        .checked_div(10_000)
        .ok_or(TradeOSError::ArithmeticOverflow)? as u64;
    let post_fee_balance = escrow_balance
        .checked_sub(fee_amount)
        .ok_or(TradeOSError::ArithmeticOverflow)?;

    let supplier_amount = (post_fee_balance as u128)
        .checked_mul(release_to_supplier_bps as u128)
        .ok_or(TradeOSError::ArithmeticOverflow)?
        .checked_div(10_000)
        .ok_or(TradeOSError::ArithmeticOverflow)? as u64;
    let buyer_amount = post_fee_balance
    ...
    if fee_amount > 0 {
        let arbiter_transfer_ctx = CpiContext::new_with_signer(
        ...
        token::transfer(arbiter_transfer_ctx, fee_amount)?;
    }
...
pub struct ResolveDispute<'info> {
    ...
    #[account(mut, constraint = arbiter_token_account.owner == escrow.arbiter)]
